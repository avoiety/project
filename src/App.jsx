...
function ProfileChoices({ label, group, choices, value, onChoose }) { 
  ...
  return <div className="profile-name">{label}<div className="choices" style={{marginTop: "5px"}}>{choices.map((choice) => <button className={value === choice ? "selected" : ""} type="button" key={choice} title={descriptions[choice]} onClick={() => onChoose(group, choice)}><i className={`swatch ${group}-${choice}`} />{labels[choice] || choice}</button>)}</div></div>; 
}
